...
if ($this->acl->has_permission('bookings','index')){	
if($order == "")
$order ="sort_order";
$data["title"]="List bookings";
$data["content"]="back_office/bookings/list";
//
...
// pagination  start :
$count_news = $this->bookings_m->getAll($this->table,$order);
$show_items = ($show_items == 'All') ? $count_news : $show_items;
$this->load->library('pagination');
$config['base_url'] = site_url("back_office/bookings/index/".$order);
$config['total_rows'] = $count_news;
$config['per_page'] = $show_items;
$config['uri_segment'] = 5;
$this->pagination->initialize($config);
$data['info'] = $this->bookings_m->list_paginate($order,$config['per_page'],$this->uri->segment(5));
// end pagination .
...
public function add(){
if ($this->acl->has_permission('bookings','add')){	
$data["title"]="Add bookings";
$data["content"]="back_office/bookings/add";
$this->load->view("back_office/template",$data);
} else {
	redirect(site_url("home/dashboard"));
}
...
if ($this->acl->has_permission('bookings','edit')){	
$data["title"]="Edit bookings";
$data["content"]="back_office/bookings/add";
$cond=array("id_bookings"=>$id);
$data["id"]=$id;
$data["info"]=$this->fct->getonerecord($this->table,$cond);
...
if ($this->acl->has_permission('bookings','delete')){
...
redirect(site_url("back_office/bookings/".$this->session->userdata("back_link")));
} else {
	redirect(site_url("home/dashboard"));
}
}
...
if ($this->acl->has_permission('bookings','delete_all')){
...
$_data=array("deleted"=>1,
"deleted_date"=>date("Y-m-d h:i:s"));
$this->db->where("id_bookings",$cehcklist[$i]);
$this->db->update($this->table,$_data);	
}
} } 
...
$this->db->where("id_bookings",$sort[$i]);
$this->db->update($this->table,$_data);	
}
}
...
$this->form_validation->set_rules("name", "name"."(".$this->lang->lang().")", "trim|required");
...
$this->form_validation->set_rules("fleet", "fleet"."(".$this->lang->lang().")", "trim");
$this->form_validation->set_rules("pickup_location", "pickup_location"."(".$this->lang->lang().")", "trim");
$this->form_validation->set_rules("dropoff_location", "dropoff_location"."(".$this->lang->lang().")", "trim");
$this->form_validation->set_rules("pickup_date", "pickup_date"."(".$this->lang->lang().")", "trim");
$this->form_validation->set_rules("dropoff_date", "dropoff_date"."(".$this->lang->lang().")", "trim");
...
$_data["id_fleet"]=$this->input->post("fleet"); 
$_data["pickup_location"]=$this->input->post("pickup_location");
$_data["dropoff_location"]=$this->input->post("dropoff_location");
$_data["pickup_date"]=$this->input->post("pickup_date");
$_data["dropoff_date"]=$this->input->post("dropoff_date");

	if($this->input->post("id")!=""){
	$_data["updated_date"]=date("Y-m-d h:i:s");
	$this->db->where("id_bookings",$this->input->post("id"));
	$this->db->update($this->table,$_data);
...
if(file_exists("./uploads/bookings/".$image)){
unlink("./uploads/bookings/".$image); }
$q=" SELECT thumb,thumb_val
FROM `content_type_attr`
WHERE id_content = (SELECT id_content FROM `content_type` WHERE name = 'bookings')
AND name = '".$field."'";
...
if(file_exists("./uploads/bookings/".$value."/".$image)){
unlink("./uploads/bookings/".$value."/".$image);	 }								
} } 
$_data[$field]="";
$this->db->where("id_bookings",$id);
$this->db->update("bookings",$_data);
echo 'Done!';
}
