<?
class Static_seo_pages extends CI_Controller{

public function __construct()
{
parent::__construct();
$this->table="static_seo_pages";
$this->load->model("static_seo_pages_m");
...
if ($this->acl->has_permission('static_seo_pages','index')){	
if($order == "")
$order ="sort_order";
$data["title"]="List static seo pages";
$data["content"]="back_office/static_seo_pages/list";
//
...
// pagination  start :
$count_news = $this->static_seo_pages_m->getAll($this->table,$order);
$show_items = ($show_items == 'All') ? $count_news : $show_items;
$this->load->library('pagination');
$config['base_url'] = site_url("back_office/static_seo_pages/index/".$order);
$config['total_rows'] = $count_news;
$config['per_page'] = $this->config->item("admin_per_page");
$config['uri_segment'] = 5;
$this->pagination->initialize($config);
$data['info'] = $this->static_seo_pages_m->list_paginate($order,$config['per_page'],$this->uri->segment(5));
// end pagination .
...
public function add(){
if ($this->acl->has_permission('static_seo_pages','add')){
$data["title"]="Add static seo pages";
$data["content"]="back_office/static_seo_pages/add";
$this->load->view("back_office/template",$data);
} else {
	redirect(site_url("home/dashboard"));
}
...
$data["title"]="Edit static seo pages";
$data["content"]="back_office/static_seo_pages/add";
$cond=array("id_static_seo_pages"=>$id);
$data["id"]=$id;
...
if ($this->acl->has_permission('static_seo_pages','delete')){
...
redirect(site_url("back_office/static_seo_pages/".$this->session->userdata("back_link")));
} else {
	redirect(site_url("home/dashboard"));
}
}
...
if ($this->acl->has_permission('static_seo_pages','delete_all')){
...
$_data=array("deleted"=>1,
"deleted_date"=>date("Y-m-d h:i:s"));
$this->db->where("id_static_seo_pages",$cehcklist[$i]);
$this->db->update($this->table,$_data);	
}
} } 
...
$this->db->where("id_static_seo_pages",$sort[$i]);
$this->db->update($this->table,$_data);		
}
}
...
$data["title"]="Add / Edit static seo pages";
...
$this->form_validation->set_rules("meta_description".$lang, "META DESCRIPTION", "trim");
$this->form_validation->set_rules("meta_keywords".$lang, "META KEYWORDS", "trim");
...
$_data["title_url".$lang]=$this->fct->cleanURL("static_seo_pages",url_title($title_url),$this->input->post("id"));
}
...
$_data["meta_keywords".$lang]=$this->input->post("meta_keywords".$lang);	
	...
	$this->db->where("id_static_seo_pages",$this->input->post("id"));
	$this->db->update($this->table,$_data);
...
if(file_exists("./uploads/static_seo_pages/".$image)){
unlink("./uploads/static_seo_pages/".$image); }
$q=" SELECT thumb,thumb_val
FROM `content_type_attr`
WHERE id_content = (SELECT id_content FROM `content_type` WHERE name = 'static seo pages')
AND name = '".$field."'";
...
if(file_exists("./uploads/static_seo_pages/".$value."/".$image)){
unlink("./uploads/static_seo_pages/".$value."/".$image);	 }								
} } 
$_data[$field]="";
$this->db->where("id_static_seo_pages",$id);
$this->db->update("static_seo_pages",$_data);
echo 'Done!';	
}
