...
if ($this->acl->has_permission('blog_tags','index')){	
if($order == "")
$order ="sort_order";
$data["title"]="List blog tags";
$data["content"]="back_office/blog_tags/list";
//
...
// pagination  start :
$count_news = $this->blog_tags_m->getAll($this->table,$order);
$show_items = ($show_items == 'All') ? $count_news : $show_items;
$this->load->library('pagination');
$config['base_url'] = site_url("back_office/blog_tags/index/".$order);
$config['total_rows'] = $count_news;
$config['per_page'] = $show_items;
$config['uri_segment'] = 5;
$this->pagination->initialize($config);
$data['info'] = $this->blog_tags_m->list_paginate($order,$config['per_page'],$this->uri->segment(5));
// end pagination .
...
public function add(){
if ($this->acl->has_permission('blog_tags','add')){	
$data["title"]="Add blog tags";
$data["content"]="back_office/blog_tags/add";
$this->load->view("back_office/template",$data);
} else {
	redirect(site_url("home/dashboard"));
}
...
if ($this->acl->has_permission('blog_tags','index')){	
$data["title"]="View blog tags";
$data["content"]="back_office/blog_tags/add";
$cond=array("id_blog_tags"=>$id);
$data["id"]=$id;
...
if ($this->acl->has_permission('blog_tags','edit')){	
$data["title"]="Edit blog tags";
$data["content"]="back_office/blog_tags/add";
$cond=array("id_blog_tags"=>$id);
$data["id"]=$id;
$data["info"]=$this->fct->getonerecord($this->table,$cond);
...
if ($this->acl->has_permission('blog_tags','delete')){
...
redirect(site_url("back_office/blog_tags/".$this->session->userdata("back_link")));
} else {
	redirect(site_url("home/dashboard"));
}
}
...
if ($this->acl->has_permission('blog_tags','delete_all')){
...
$_data=array("deleted"=>1,
"deleted_date"=>date("Y-m-d h:i:s"));
$this->db->where("id_blog_tags",$cehcklist[$i]);
$this->db->update($this->table,$_data);	
}
} } 
...
$this->db->where("id_blog_tags",$sort[$i]);
$this->db->update($this->table,$_data);	
}
}
...
$_data["meta_keywords".$lang]=$this->input->post("meta_keywords".$lang);	
	...
	$this->db->where("id_blog_tags",$this->input->post("id"));
	$this->db->update($this->table,$_data);
...
if(file_exists("./uploads/blog_tags/".$image)){
unlink("./uploads/blog_tags/".$image); }
$q=" SELECT thumb,thumb_val
FROM `content_type_attr`
WHERE id_content = (SELECT id_content FROM `content_type` WHERE name = 'blog tags')
AND name = '".$field."'";
...
if(file_exists("./uploads/blog_tags/".$value."/".$image)){
unlink("./uploads/blog_tags/".$value."/".$image);	 }								
} } 
$_data[$field]="";
$this->db->where("id_blog_tags",$id);
$this->db->update("blog_tags",$_data);
echo 'Done!';
}
