...
if ($this->acl->has_permission('faq','index')){	
if($order == "")
$order ="sort_order";
$data["title"]="List faq";
$data["content"]="back_office/faq/list";
//
...
// pagination  start :
$count_news = $this->faq_m->getAll($this->table,$order);
$show_items = ($show_items == 'All') ? $count_news : $show_items;
$this->load->library('pagination');
$config['base_url'] = site_url("back_office/faq/index/".$order);
$config['total_rows'] = $count_news;
$config['per_page'] = $show_items;
$config['uri_segment'] = 5;
$this->pagination->initialize($config);
$data['info'] = $this->faq_m->list_paginate($order,$config['per_page'],$this->uri->segment(5));
// end pagination .
...
$data["title"]="Add faq";
$data["content"]="back_office/faq/add";
$this->load->view("back_office/template",$data);
} else {
	redirect(site_url("home/dashboard"));
}
...
redirect(site_url("back_office/faq/".$this->session->userdata("back_link")));
} else {
	redirect(site_url("home/dashboard"));
}
}
...
$_data=array("deleted"=>1,
"deleted_date"=>date("Y-m-d h:i:s"));
$this->db->where("id_faq",$cehcklist[$i]);
$this->db->update($this->table,$_data);	
}
} } 
...
$this->form_validation->set_rules("answer", "answer"."(".$this->lang->lang().")", "trim|required");
...
if(file_exists("./uploads/faq/".$image)){
unlink("./uploads/faq/".$image); }
$q=" SELECT thumb,thumb_val
FROM `content_type_attr`
WHERE id_content = (SELECT id_content FROM `content_type` WHERE name = 'faq')
AND name = '".$field."'";
...
if(file_exists("./uploads/faq/".$value."/".$image)){
unlink("./uploads/faq/".$value."/".$image);	 }								
} } 
$_data[$field]="";
$this->db->where("id_faq",$id);
$this->db->update("faq",$_data);
echo 'Done!';
}
