<?
class Static_translations extends CI_Controller{

public function __construct()
{
parent::__construct();
$this->table="static_translations";
$this->load->model("static_translations_m");
...
if ($this->acl->has_permission('static_translations','index')){	
if($order == "")
$order ="sort_order";
$data["title"]="List static translations";
$data["content"]="back_office/static_translations/list";
//
$this->session->unset_userdata("back_link");
//
$this->session->set_userdata('back_link','index/'.$order.'/'.$this->uri->segment(5));
$data["show_items"] = $this->config->item("admin_per_page");
// pagination  start :
//$count_news = $this->static_translations_m->getAll($this->table,$order);
//$show_items = ($show_items == 'All') ? $count_news : $show_items;
/*$this->load->library('pagination');
$config['base_url'] = site_url("back_office/static_translations/index/".$order);
$config['total_rows'] = $count_news;
$config['per_page'] = $this->config->item("admin_per_page");
$config['uri_segment'] = 5;
$this->pagination->initialize($config);
$data['info'] = $this->static_translations_m->list_paginate($order,$config['per_page'],$this->uri->segment(5));*/

// organize data filtration
$cond = array();
$like = array();
$url = site_url('back_office/static_translations').'?pagination=on';

if($this->input->get("title") != "") {
	$title = $this->input->get("title");
	$url .= '&title='.$title;
	if(!empty($title))
	$like['UPPER(static_translations.title'.getFieldLanguage($this->lang->lang()).")"] = strtoupper($title);
}
// pagination  start :
$count_news = $this->static_translations_m->getStaticTranslations($cond,$like);
//$show_items = ($show_items == 'All') ? $count_news : $show_items;
$this->load->library('pagination');
$config['base_url'] = $url;
$config['num_links'] = '8';
...
$data['info'] = $this->static_translations_m->getStaticTranslations($cond,$like,$config['per_page'],$page);
...
public function add(){
if ($this->acl->has_permission('static_translations','add')){	
$data["title"]="Add static translations";
$data["content"]="back_office/static_translations/add";
$this->load->view("back_office/template",$data);
} else {
	redirect(site_url("home/dashboard"));
}
...
if ($this->acl->has_permission('static_translations','index')){	
$data["title"]="View static translations";
$data["content"]="back_office/static_translations/add";
$cond=array("id_static_translations"=>$id);
$data["id"]=$id;
...
if ($this->acl->has_permission('static_translations','edit')){	
$data["title"]="Edit static translations";
$data["content"]="back_office/static_translations/add";
$cond=array("id_static_translations"=>$id);
$data["id"]=$id;
$data["info"]=$this->fct->getonerecord($this->table,$cond);
...
if ($this->acl->has_permission('static_translations','delete')){
...
redirect(site_url("back_office/static_translations/".$this->session->userdata("back_link")));
} else {
	redirect(site_url("home/dashboard"));
}
}
...
if ($this->acl->has_permission('static_translations','delete_all')){
...
$_data=array("deleted"=>1,
"deleted_date"=>date("Y-m-d h:i:s"));
$this->db->where("id_static_translations",$cehcklist[$i]);
$this->db->update($this->table,$_data);	
}
} } 
...
$this->db->where("id_static_translations",$sort[$i]);
$this->db->update($this->table,$_data);	
}
}
...
$data["title"]="Add / Edit static translations";
...
//$this->form_validation->set_rules("key_index", "key index"."(".$this->lang->lang().")", "trim|required");
...
//$_data["key_index"]=$this->input->post("key_index");

	if($this->input->post("id")!=""){
	$_data["updated_date"]=date("Y-m-d h:i:s");
	$this->db->where("id_static_translations",$this->input->post("id"));
	$this->db->update($this->table,$_data);
...
if(file_exists("./uploads/static_translations/".$image)){
unlink("./uploads/static_translations/".$image); }
$q=" SELECT thumb,thumb_val
FROM `content_type_attr`
WHERE id_content = (SELECT id_content FROM `content_type` WHERE name = 'static translations')
AND name = '".$field."'";
...
if(file_exists("./uploads/static_translations/".$value."/".$image)){
unlink("./uploads/static_translations/".$value."/".$image);	 }								
} } 
$_data[$field]="";
$this->db->where("id_static_translations",$id);
$this->db->update("static_translations",$_data);
echo 'Done!';
}
