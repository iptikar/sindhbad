...
{
parent::__construct();
$this->table="pickup_location";
$this->load->model("pickup_location_m");
...
if ($this->acl->has_permission('pickup_location','index')){	
if($order == "")
$order ="sort_order";
$data["title"]="List pickup location";
$data["content"]="back_office/pickup_location/list";
//
...
// pagination  start :
$count_news = $this->pickup_location_m->getAll($this->table,$order);
$show_items = ($show_items == 'All') ? $count_news : $show_items;
$this->load->library('pagination');
$config['base_url'] = site_url("back_office/pickup_location/index/".$order);
$config['total_rows'] = $count_news;
$config['per_page'] = $show_items;
$config['uri_segment'] = 5;
$this->pagination->initialize($config);
$data['info'] = $this->pickup_location_m->list_paginate($order,$config['per_page'],$this->uri->segment(5));
// end pagination .
...
public function add(){
if ($this->acl->has_permission('pickup_location','add')){	
$data["title"]="Add pickup location";
$data["content"]="back_office/pickup_location/add";
$this->load->view("back_office/template",$data);
} else {
	redirect(site_url("home/dashboard"));
}
...
if ($this->acl->has_permission('pickup_location','index')){	
$data["title"]="View pickup location";
$data["content"]="back_office/pickup_location/add";
$cond=array("id_pickup_location"=>$id);
$data["id"]=$id;
...
if ($this->acl->has_permission('pickup_location','edit')){	
$data["title"]="Edit pickup location";
$data["content"]="back_office/pickup_location/add";
$cond=array("id_pickup_location"=>$id);
$data["id"]=$id;
$data["info"]=$this->fct->getonerecord($this->table,$cond);
...
if ($this->acl->has_permission('pickup_location','delete')){
...
redirect(site_url("back_office/pickup_location/".$this->session->userdata("back_link")));
} else {
	redirect(site_url("home/dashboard"));
}
}
...
if ($this->acl->has_permission('pickup_location','delete_all')){
...
$_data=array("deleted"=>1,
"deleted_date"=>date("Y-m-d h:i:s"));
$this->db->where("id_pickup_location",$cehcklist[$i]);
$this->db->update($this->table,$_data);	
}
} } 
...
$this->db->where("id_pickup_location",$sort[$i]);
$this->db->update($this->table,$_data);	
}
}
...
$data["title"]="Add / Edit pickup location";
...
$_data["meta_keywords".$lang]=$this->input->post("meta_keywords".$lang);	
	...
	$this->db->where("id_pickup_location",$this->input->post("id"));
	$this->db->update($this->table,$_data);
...
if(file_exists("./uploads/pickup_location/".$image)){
unlink("./uploads/pickup_location/".$image); }
$q=" SELECT thumb,thumb_val
FROM `content_type_attr`
WHERE id_content = (SELECT id_content FROM `content_type` WHERE name = 'pickup location')
AND name = '".$field."'";
...
if(file_exists("./uploads/pickup_location/".$value."/".$image)){
unlink("./uploads/pickup_location/".$value."/".$image);	 }								
} } 
$_data[$field]="";
$this->db->where("id_pickup_location",$id);
$this->db->update("pickup_location",$_data);
echo 'Done!';
}
