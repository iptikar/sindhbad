...
if ($this->acl->has_permission('','index')){	
if($order == "")
$order ="sort_order";
$data["title"]="List ";
$data["content"]="back_office//list";
//
...
// pagination  start :
$count_news = $this->_m->getAll($this->table,$order);
$show_items = ($show_items == 'All') ? $count_news : $show_items;
$this->load->library('pagination');
$config['base_url'] = site_url("back_office//index/".$order);
$config['total_rows'] = $count_news;
$config['per_page'] = $this->config->item("admin_per_page");
$config['uri_segment'] = 5;
$this->pagination->initialize($config);
$data['info'] = $this->_m->list_paginate($order,$config['per_page'],$this->uri->segment(5));
// end pagination .
...
redirect(site_url("back_office//".$this->session->userdata("back_link")));
} else {
	redirect(site_url("home/dashboard"));
}
}
...
$_data["meta_keywords".$lang]=$this->input->post("meta_keywords".$lang);	
	...
	$this->db->where("id_",$this->input->post("id"));
	$this->db->update($this->table,$_data);
...
WHERE id_content = (SELECT id_content FROM `content_type` WHERE name = '')
AND name = '".$field."'";
