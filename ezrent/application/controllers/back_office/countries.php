...
if ($this->acl->has_permission('countries','index')){	
if($order == "")
$order ="sort_order";
$data["title"]="List countries";
$data["content"]="back_office/countries/list";
//
...
// pagination  start :
$count_news = $this->countries_m->getAll($this->table,$order);
$show_items = ($show_items == 'All') ? $count_news : $show_items;
$this->load->library('pagination');
$config['base_url'] = site_url("back_office/countries/index/".$order);
$config['total_rows'] = $count_news;
$config['per_page'] = $this->config->item("admin_per_page");
$config['uri_segment'] = 5;
$this->pagination->initialize($config);
$data['info'] = $this->countries_m->list_paginate($order,$config['per_page'],$this->uri->segment(5));
// end pagination .
...
$data["content"]="back_office/countries/add";
$this->load->view("back_office/template",$data);
} else {
	redirect(site_url("home/dashboard"));
}
...
redirect(site_url("back_office/countries/".$this->session->userdata("back_link")));
} else {
	redirect(site_url("home/dashboard"));
}
}
...
$_data=array("deleted"=>1,
"deleted_date"=>date("Y-m-d h:i:s"));
$this->db->where("id_countries",$cehcklist[$i]);
$this->db->update($this->table,$_data);	
}
} } 
...
$this->db->where("id_countries",$sort[$i]);
$this->db->update($this->table,$_data);	
}
}
...
$_data["meta_keywords".$lang]=$this->input->post("meta_keywords".$lang);	
	...
	$this->db->where("id_countries",$this->input->post("id"));
	$this->db->update($this->table,$_data);
...
if(file_exists("./uploads/countries/".$image)){
unlink("./uploads/countries/".$image); }
$q=" SELECT thumb,thumb_val
FROM `content_type_attr`
WHERE id_content = (SELECT id_content FROM `content_type` WHERE name = 'countries')
AND name = '".$field."'";
...
if(file_exists("./uploads/countries/".$value."/".$image)){
unlink("./uploads/countries/".$value."/".$image);	 }								
} } 
$_data[$field]="";
$this->db->where("id_countries",$id);
$this->db->update("countries",$_data);
echo 'Done!';
}
