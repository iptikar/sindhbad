...
if ($this->acl->has_permission('category','index')){	
if($order == "")
$order ="sort_order";
$data["title"]="List category";
$data["content"]="back_office/category/list";
//
...
// pagination  start :
$count_news = $this->category_m->getAll($this->table,$order);
$show_items = ($show_items == 'All') ? $count_news : $show_items;
$this->load->library('pagination');
$config['base_url'] = site_url("back_office/category/index/".$order);
$config['total_rows'] = $count_news;
$config['per_page'] = $show_items;
$config['uri_segment'] = 5;
$this->pagination->initialize($config);
$data['info'] = $this->category_m->list_paginate($order,$config['per_page'],$this->uri->segment(5));
// end pagination .
...
$data["content"]="back_office/category/add";
$this->load->view("back_office/template",$data);
} else {
	redirect(site_url("home/dashboard"));
}
...
redirect(site_url("back_office/category/".$this->session->userdata("back_link")));
} else {
	redirect(site_url("home/dashboard"));
}
}
...
$_data=array("deleted"=>1,
"deleted_date"=>date("Y-m-d h:i:s"));
$this->db->where("id_category",$cehcklist[$i]);
$this->db->update($this->table,$_data);	
}
} } 
...
$this->db->where("id_category",$sort[$i]);
$this->db->update($this->table,$_data);	
}
}
...
WHERE id_content = (SELECT id_content FROM `content_type` WHERE name = 'category')
AND name = '".$field."'";
