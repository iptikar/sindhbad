...
window.location="<?= site_url("back_office/category/delete"); ?>/"+id;
return false;
}
...
$("#result").load("<?= site_url("back_office/category/sorted"); ?>"+"?"+ser);
}
});
...
<? if ($this->acl->has_permission('category','add')){ ?>
<li class="pull-right">
<a href="<?= site_url('back_office/category/add'); ?>" id="topLink" class="btn btn-info top_btn" title="">Add category</a>
</li><? } ?>
</ul> 
</div>
...
echo form_open_multipart('back_office/category/delete_all', $attributes); 
?>  		
...
<td width="2%">&nbsp;</td>
<th>
<a href="<?= site_url('back_office/category/index/title'); ?>" class="<? if($sego == "title") echo 'green'; else echo 'order_link'; ?>" >
TITLE
</a>
...
<td colspan="2">
<? if ($this->acl->has_permission('category','delete_all')){ ?>
...
$i++; 
?>
<tr id="<?=$val["id_category"]; ?>">
<td class="col-chk"><input type="checkbox" name="cehcklist[]" value="<?= $val["id_category"] ; ?>" /></td>
<td class="title_search">
<? echo $val["title".$lang]; ?>
</td>
<td style="text-align:center"><? if ($this->acl->has_permission('category','index')){ ?>
<a href="<?= site_url('back_office/category/view/'.$val["id_category"]);?>" class="table-edit-link" title="View" >
<i class="icon-search" ></i> View</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('category','edit')){ ?>
<a href="<?= site_url('back_office/category/edit/'.$val["id_category"]);?>" class="table-edit-link" title="Edit" >
<i class="icon-edit" ></i> Edit</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('category','delete')){ ?>
<a onclick="if(confirm('Are you sure you want delete this item ?')){ delete_row('<?=$val["id_category"];?>'); }" class="table-delete-link cur" title="Delete" >
...
<form action="<?= site_url("back_office/category"); ?>" method="post"  id="show_items">
Show Items&nbsp;
