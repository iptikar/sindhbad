...
$(document).ready(function(){
        $("select[name='related_news_select[]']").multiselect().multiselectfilter();
});
</script>
...
$ul = array(
            anchor('back_office/news/'.$this->session->userdata("back_link"),'<b>List news</b>').'<span class="divider">/</span>',
            $title => array('li_attributes' => 'class = "active"', 'contents' => $title),
            );
if($this->config->item("language_module") && isset($id)) {
			   $ul['translate'] = array('li_attributes' => 'class = "pull-right"', 'contents' => '<a href="'.site_url('back_office/translation/section/news/'.$id).'" class="btn btn-info top_btn">Translate</a>');
			}
...
echo form_open_multipart('back_office/news/submit', $attributes); 
...
//NEWS DATE SECTION.
echo form_label('<b>NEWS DATE</b>', 'NEWS DATE');
echo '<div class="input-append date" data-date="" data-date-format="dd/mm/yyyy">';
echo form_input(array('name' => 'news_date', 'value' => set_value("news_date",$this->fct->date_out_formate($info["news_date"])),'class' =>'input-small','size'=>16 ));
echo '<span class="add-on"><i class="icon-th"></i></span></div>';
echo form_error("news_date","<span class='text-error'>","</span>");
echo br();
...
echo "<span >";
if($info["picture"] != ""){ 
echo '<span id="picture_'.$info["id_news"].'">'.anchor('uploads/news/'.$info["picture"],'show image',array("class" => 'blue gallery'));
echo nbs(3);?>
<a class="cur" onclick='removeFile("news","picture","<?php echo $info["picture"]; ?>",<?php echo $info["id_news"]; ?>)'><img src="<?php echo base_url()?>images/delete.png" /></a></span>
<?php
...
echo form_label('<b>RELATED NEWS</b>', 'RELATED NEWS');
$related_ids = array();
$lang_select = $this->lang->lang();
if(isset($id)) {
$rp = $this->fct->getRelatedNews($id);

if(!empty($rp)) {
foreach($rp as $rp2)
array_push($related_ids,$rp2['id_news']);
}
}
if(isset($lang)) {
	$lang_select = $lang;
}
$allnews = $this->fct->getAll('news','sort_order');
$options_news = array();
foreach($allnews as $news) {
	$options_news[$news['id_news']] = $news['title'];
}
?>
<label>
<?php
echo form_multiselect('related_news_select[]',$options_news,$related_ids); 
echo form_error("related_news_select","<span class='text-error'>","</span>");
...
echo '<label>';
foreach($rp as $rp1) {
echo '<a href="'.site_url("back_office/news/edit/".$rp1['id_news']).'">'.$rp1['title'].'</a><br />';
}
echo '</label>';
}
}
