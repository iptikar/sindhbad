...
$ul = array(
            anchor('back_office/static_seo_pages/'.$this->session->userdata("back_link"),'<b>List static seo pages</b>').'<span class="divider">/</span>',
            $title => array('li_attributes' => 'class = "active"', 'contents' => $title),
            );
if($this->config->item("language_module") && isset($id)) {
			   $ul['translate'] = array('li_attributes' => 'class = "pull-right"', 'contents' => '<a href="'.site_url('back_office/translation/section/static_seo_pages/'.$id).'" class="btn btn-info top_btn">Translate</a>');
			}
$ul_attributes = array(
                    'class' => 'breadcrumb'
                    );
...
echo form_open_multipart('back_office/static_seo_pages/submit', $attributes); 
