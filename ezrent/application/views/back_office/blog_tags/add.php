...
$ul = array(
            anchor('back_office/blog_tags/'.$this->session->userdata("back_link"),'<b>List blog tags</b>').'<span class="divider">/</span>',
            $title => array('li_attributes' => 'class = "active"', 'contents' => $title),
            );
if($this->config->item("language_module") && isset($id)) {
			   $ul['translate'] = array('li_attributes' => 'class = "pull-right"', 'contents' => '<a href="'.site_url('back_office/translation/section/blog_tags/'.$id).'" class="btn btn-info top_btn">Translate</a>');
			}
...
echo form_open_multipart('back_office/blog_tags/submit', $attributes); 
