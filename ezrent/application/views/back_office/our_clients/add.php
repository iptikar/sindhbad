...
$ul = array(
            anchor('back_office/our_clients/'.$this->session->userdata("back_link"),'<b>List our clients</b>').'<span class="divider">/</span>',
            $title => array('li_attributes' => 'class = "active"', 'contents' => $title),
            );
if($this->config->item("language_module") && isset($id)) {
			   $ul['translate'] = array('li_attributes' => 'class = "pull-right"', 'contents' => '<a href="'.site_url('back_office/translation/section/our_clients/'.$id).'" class="btn btn-info top_btn">Translate</a>');
			}
...
echo form_open_multipart('back_office/our_clients/submit', $attributes); 
...
echo "<span >";
if($info["logo"] != ""){ 
echo '<span id="logo_'.$info["id_our_clients"].'">'.anchor('uploads/our_clients/'.$info["logo"],'show image',array("class" => 'blue gallery'));
echo nbs(3);?>
<a class="cur" onclick='removeFile("our_clients","logo","<?php echo $info["logo"]; ?>",<?php echo $info["id_our_clients"]; ?>)'><img src="<?php echo base_url()?>images/delete.png" /></a></span>
<?php
...
echo form_label('<b>Logo Title&nbsp;:</b>', 'Title');
...
echo form_label('<b>Logo Alt&nbsp;:</b>', 'Title');
