...
window.location="<?= site_url("back_office/faq/delete"); ?>/"+id;
return false;
}
...
$("#result").load("<?= site_url("back_office/faq/sorted"); ?>"+"?"+ser);
}
});
...
<? if ($this->acl->has_permission('faq','add')){ ?>
<li class="pull-right">
<a href="<?= site_url('back_office/faq/add'); ?>" id="topLink" class="btn btn-info top_btn" title="">Add faq</a>
</li><? } ?>
</ul> 
</div>
...
echo form_open_multipart('back_office/faq/delete_all', $attributes); 
?>  		
...
<td width="2%">&nbsp;</td>
<th>
<a href="<?= site_url('back_office/faq/index/title'); ?>" class="<? if($sego == "title") echo 'green'; else echo 'order_link'; ?>" >
TITLE
</a>
...
<td colspan="2">
<? if ($this->acl->has_permission('faq','delete_all')){ ?>
...
?>
<tr id="<?=$val["id_faq"]; ?>">
<td class="col-chk"><input type="checkbox" name="cehcklist[]" value="<?= $val["id_faq"] ; ?>" /></td>
<td class="title_search">
<? echo $val["title".$lang]; ?>
</td>
<td style="text-align:center"><? if ($this->acl->has_permission('faq','index')){ ?>
<a href="<?= site_url('back_office/faq/view/'.$val["id_faq"]);?>" class="table-edit-link" title="View" >
<i class="icon-search" ></i> View</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('faq','edit')){ ?>
<a href="<?= site_url('back_office/faq/edit/'.$val["id_faq"]);?>" class="table-edit-link" title="Edit" >
<i class="icon-edit" ></i> Edit</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('faq','delete')){ ?>
<a onclick="if(confirm('Are you sure you want delete this item ?')){ delete_row('<?=$val["id_faq"];?>'); }" class="table-delete-link cur" title="Delete" >
...
<form action="<?= site_url("back_office/faq"); ?>" method="post"  id="show_items">
Show Items&nbsp;
