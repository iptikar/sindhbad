...
window.location="<?= site_url("back_office/fleet/delete"); ?>/"+id;
return false;
}
...
$("#result").load("<?= site_url("back_office/fleet/sorted"); ?>"+"?"+ser);
}
});
...
<li class="pull-right">
<a href="<?= site_url('back_office/fleet/add'); ?>" id="topLink" class="btn btn-info top_btn" title="">Add fleet</a>
</li><? } ?>
</ul> 
</div>
...
echo form_open_multipart('back_office/fleet/delete_all', $attributes); 
?>  		
...
<td width="2%">&nbsp;</td>
<th>
<a href="<?= site_url('back_office/fleet/index/title'); ?>" class="<? if($sego == "title") echo 'green'; else echo 'order_link'; ?>" >
TITLE
</a>
</th>
<th>CATEGORY</th><th style="text-align:center;" width="250">ACTION</th></tr>
<tr>
<td></td>
<td><input type="text" name="search" class="search_box" id="title_search" /></td>

<td><input type="text" name="search" class="search_box" id="category_search" /></td>
<td></td>
</tr>
</thead><tfoot><tr>
...
?>
<tr id="<?=$val["id_fleet"]; ?>">
<td class="col-chk"><input type="checkbox" name="cehcklist[]" value="<?= $val["id_fleet"] ; ?>" /></td>
<td class="title_search">
<? echo $val["title".$lang]; ?>
</td>
<td class="category_search">
<? if($val["id_category"] != 0){
$cond=array("id_category" => $val["id_category"]);
echo $this->fct->getonecell("category","title",$cond); } else { echo "<small>Not available</small>"; }   ?>
</td>
<td style="text-align:center"><a href="<?= site_url("back_office/control/manage_gallery/fleet/".$val["id_fleet"]); ?>" title="Add Photos">
<i class="icon-film"></i> Manage Gallery</a>&nbsp;<? if ($this->acl->has_permission('fleet','index')){ ?>
<a href="<?= site_url('back_office/fleet/view/'.$val["id_fleet"]);?>" class="table-edit-link" title="View" >
<i class="icon-search" ></i> View</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('fleet','edit')){ ?>
<a href="<?= site_url('back_office/fleet/edit/'.$val["id_fleet"]);?>" class="table-edit-link" title="Edit" >
<i class="icon-edit" ></i> Edit</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('fleet','delete')){ ?>
<a onclick="if(confirm('Are you sure you want delete this item ?')){ delete_row('<?=$val["id_fleet"];?>'); }" class="table-delete-link cur" title="Delete" >
...
<form action="<?= site_url("back_office/fleet"); ?>" method="post"  id="show_items">
Show Items&nbsp;
