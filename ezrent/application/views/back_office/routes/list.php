...
window.location="<?= base_url(); ?>back_office/routes/delete/"+id;
return false;
}
...
$("#result").load("<?= base_url(); ?>back_office/routes/sorted?"+ser);
}
});
...
<li class="pull-right">
<a href="<?= site_url('back_office/routes/add'); ?>" id="topLink" class="btn btn-info top_btn" title="">Add routes</a>
</li><? } ?>
<? if ($this->acl->has_permission('routes','edit')){ ?>
<li class="pull-right" style="margin-right:10px">
<a onclick="if(confirm('are you sure?')) { window.location = '<?= site_url('back_office/routes/updateAll1'); ?>';}" id="topLink" class="btn btn-info top_btn" title="">Update All</a>
</li><? } ?>
</ul> 
</div>
...
<td><input type="text" name="search" class="search_box" id="route_index_search" /></td>
<td><input type="text" name="search" class="search_box" id="route_rule_search" /></td>
<td></td>
</tr>
</thead><tfoot><tr>
...
?>
<tr id="<?=$val["id_routes"]; ?>">
<td class="col-chk"><input type="checkbox" name="cehcklist[]" value="<?= $val["id_routes"] ; ?>" /></td>
<td class="title_search">
<? echo $val["title"]; ?>
</td>
<td class="route_index_search">
<? echo $val["route_index"]; ?>
</td>
<td class="route_rule_search">
<? echo $val["route_rule"]; ?>
</td>
<td style="text-align:center">
<? if($this->config->item("language_module")) { ?>
<!--<a href="<?= site_url('back_office/translation/section/routes/'.$val["id_routes"]);?>" class="table-edit-link" title="Translate" >
<i class="icon-search" ></i> Translate</a> <span class="hidden"> | </span>-->
<? }?>
<? if ($this->acl->has_permission('routes','index')){ ?>
<a href="<?= site_url('back_office/routes/view/'.$val["id_routes"]);?>" class="table-edit-link" title="View" >
<i class="icon-search" ></i> View</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('routes','edit')){ ?>
<a href="<?= site_url('back_office/routes/edit/'.$val["id_routes"]);?>" class="table-edit-link" title="Edit" >
<i class="icon-edit" ></i> Edit</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('routes','delete')){ ?>
<a onclick="if(confirm('Are you sure you want delete this item ?')){ delete_row('<?=$val["id_routes"];?>'); }" class="table-delete-link cur" title="Delete" >
