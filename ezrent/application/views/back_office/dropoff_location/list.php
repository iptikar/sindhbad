...
window.location="<?= site_url("back_office/dropoff_location/delete"); ?>/"+id;
return false;
}
...
$("#result").load("<?= site_url("back_office/dropoff_location/sorted"); ?>"+"?"+ser);
}
});
...
<li class="pull-right">
<a href="<?= site_url('back_office/dropoff_location/add'); ?>" id="topLink" class="btn btn-info top_btn" title="">Add dropoff location</a>
</li><? } ?>
</ul> 
</div>
...
echo form_open_multipart('back_office/dropoff_location/delete_all', $attributes); 
?>  		
...
<td width="2%">&nbsp;</td>
<th>
<a href="<?= site_url('back_office/dropoff_location/index/title'); ?>" class="<? if($sego == "title") echo 'green'; else echo 'order_link'; ?>" >
TITLE
</a>
...
$i++; 
?>
<tr id="<?=$val["id_dropoff_location"]; ?>">
<td class="col-chk"><input type="checkbox" name="cehcklist[]" value="<?= $val["id_dropoff_location"] ; ?>" /></td>
<td class="title_search">
<? echo $val["title".$lang]; ?>
</td>
<td style="text-align:center"><? if ($this->acl->has_permission('dropoff_location','index')){ ?>
<a href="<?= site_url('back_office/dropoff_location/view/'.$val["id_dropoff_location"]);?>" class="table-edit-link" title="View" >
<i class="icon-search" ></i> View</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('dropoff_location','edit')){ ?>
<a href="<?= site_url('back_office/dropoff_location/edit/'.$val["id_dropoff_location"]);?>" class="table-edit-link" title="Edit" >
<i class="icon-edit" ></i> Edit</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('dropoff_location','delete')){ ?>
<a onclick="if(confirm('Are you sure you want delete this item ?')){ delete_row('<?=$val["id_dropoff_location"];?>'); }" class="table-delete-link cur" title="Delete" >
...
<form action="<?= site_url("back_office/dropoff_location"); ?>" method="post"  id="show_items">
Show Items&nbsp;
