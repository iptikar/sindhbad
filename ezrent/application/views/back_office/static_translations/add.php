...
$ul = array(
            anchor('back_office/static_translations/'.$this->session->userdata("back_link"),'<b>List static translations</b>').'<span class="divider">/</span>',
            $title => array('li_attributes' => 'class = "active"', 'contents' => $title),
            );
if($this->config->item("language_module") && isset($id)) {
			   $ul['translate'] = array('li_attributes' => 'class = "pull-right"', 'contents' => '<a href="'.site_url('back_office/translation/section/static_translations/'.$id).'" class="btn btn-info top_btn">Translate</a>');
			}
...
echo form_open_multipart('back_office/static_translations/submit', $attributes); 
...
echo br();
*/
//KEY INDEX&nbsp;<em>*</em>: SECTION.
/*echo form_label('<b>KEY INDEX&nbsp;<em>*</em>:</b>', 'KEY INDEX&nbsp;<em>*</em>:');
echo form_input(array('name' => 'key_index', 'value' => set_value("key_index",$info["key_index"]),'class' =>'span' ));
echo form_error("key_index","<span class='text-error'>","</span>");
echo br();
echo br();*/
if ($this->uri->segment(3) != "view" ){
echo '<p class="pull-right">';
echo form_submit(array('name' => 'submit1','value' => 'Save Changes','class' => 'btn btn-primary') );
echo '</p>';
}
