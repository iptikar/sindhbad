...
window.location="<?= site_url("back_office/countries/delete"); ?>/"+id;
return false;
}
...
$("#result").load("<?= site_url("back_office/countries/sorted"); ?>"+"?"+ser);
}
});
...
<? if ($this->acl->has_permission('countries','add')){ ?>
<li class="pull-right">
<a href="<?= site_url('back_office/countries/add'); ?>" id="topLink" class="btn btn-info top_btn" title="">Add countries</a>
</li><? } ?>
</ul> 
</div>
...
echo form_open_multipart('back_office/countries/delete_all', $attributes); 
?>  		
...
<td width="2%">&nbsp;</td>
<th>
<a href="<?= site_url('back_office/countries/index/title'); ?>" class="<? if($sego == "title") echo 'green'; else echo 'order_link'; ?>" >
TITLE
</a>
...
<td colspan="2">
<? if ($this->acl->has_permission('countries','delete_all')){ ?>
...
$i++; 
?>
<tr id="<?=$val["id_countries"]; ?>">
<td class="col-chk"><input type="checkbox" name="cehcklist[]" value="<?= $val["id_countries"] ; ?>" /></td>
<td class="title_search">
<? echo $val["title".$lang]; ?>
</td>
<td style="text-align:center"><? if ($this->acl->has_permission('countries','index')){ ?>
<a href="<?= site_url('back_office/countries/view/'.$val["id_countries"]);?>" class="table-edit-link" title="View" >
<i class="icon-search" ></i> View</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('countries','edit')){ ?>
<a href="<?= site_url('back_office/countries/edit/'.$val["id_countries"]);?>" class="table-edit-link" title="Edit" >
<i class="icon-edit" ></i> Edit</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('countries','delete')){ ?>
<a onclick="if(confirm('Are you sure you want delete this item ?')){ delete_row('<?=$val["id_countries"];?>'); }" class="table-delete-link cur" title="Delete" >
...
<form action="<?= site_url("back_office/countries"); ?>" method="post"  id="show_items">
Show Items&nbsp;
