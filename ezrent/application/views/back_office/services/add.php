...
$ul = array(
            anchor('back_office/services/'.$this->session->userdata("back_link"),'<b>List services</b>').'<span class="divider">/</span>',
            $title => array('li_attributes' => 'class = "active"', 'contents' => $title),
            );
if($this->config->item("language_module") && isset($id)) {
			   $ul['translate'] = array('li_attributes' => 'class = "pull-right"', 'contents' => '<a href="'.site_url('back_office/translation/section/services/'.$id).'" class="btn btn-info top_btn">Translate</a>');
			}
...
echo form_open_multipart('back_office/services/submit', $attributes); 
...
echo "<span >";
if($info["picture"] != ""){ 
echo '<span id="picture_'.$info["id_services"].'">'.anchor('uploads/services/'.$info["picture"],'show image',array("class" => 'blue gallery'));
echo nbs(3);?>
<a class="cur" onclick='removeFile("services","picture","<?php echo $info["picture"]; ?>",<?php echo $info["id_services"]; ?>)'><img src="<?php echo base_url()?>images/delete.png" /></a></span>
<?php
