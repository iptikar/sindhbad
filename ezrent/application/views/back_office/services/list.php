...
window.location="<?= site_url("back_office/services/delete"); ?>/"+id;
return false;
}
...
$("#result").load("<?= site_url("back_office/services/sorted"); ?>"+"?"+ser);
}
});
...
<li class="pull-right">
<a href="<?= site_url('back_office/services/add'); ?>" id="topLink" class="btn btn-info top_btn" title="">Add services</a>
</li><? } ?>
</ul> 
</div>
...
echo form_open_multipart('back_office/services/delete_all', $attributes); 
?>  		
...
<td width="2%">&nbsp;</td>
<th>
<a href="<?= site_url('back_office/services/index/title'); ?>" class="<? if($sego == "title") echo 'green'; else echo 'order_link'; ?>" >
TITLE
</a>
...
$i++; 
?>
<tr id="<?=$val["id_services"]; ?>">
<td class="col-chk"><input type="checkbox" name="cehcklist[]" value="<?= $val["id_services"] ; ?>" /></td>
<td class="title_search">
<? echo $val["title".$lang]; ?>
</td>
<td style="text-align:center"><? if ($this->acl->has_permission('services','index')){ ?>
<a href="<?= site_url('back_office/services/view/'.$val["id_services"]);?>" class="table-edit-link" title="View" >
<i class="icon-search" ></i> View</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('services','edit')){ ?>
<a href="<?= site_url('back_office/services/edit/'.$val["id_services"]);?>" class="table-edit-link" title="Edit" >
<i class="icon-edit" ></i> Edit</a> <span class="hidden"> | </span>
<? } ?>
<? if ($this->acl->has_permission('services','delete')){ ?>
<a onclick="if(confirm('Are you sure you want delete this item ?')){ delete_row('<?=$val["id_services"];?>'); }" class="table-delete-link cur" title="Delete" >
...
<form action="<?= site_url("back_office/services"); ?>" method="post"  id="show_items">
Show Items&nbsp;
