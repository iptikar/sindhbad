...
class MY_Lang extends CI_Lang {
	...
	***************************************************/
	...
	function switch_uri($lang,$uri_string = '')
	{
		$CI =& get_instance();
		if($uri_string != '') {
			$uri = $uri_string;
		...
		if ($uri != "")
		{
			...
			if(!empty($exploded)) {
				foreach($exploded as $exp) {
					if(!empty($exp)) {
						array_push($new_exploded,$exp);
					}
				}
			}
			$exploded = $new_exploded;
			//print '<pre>';print_r($exploded);exit;
			if($this->lang() == $lang) {
				// do nothing
			}
			else {
				if(isset($exploded[0]) && !in_array($exploded[0],$all_languages)) {
	...
	function localized($uri)
	{
		if($this->has_language($uri)
				|| $this->is_special($uri)
				|| preg_match('/(.+)\.[a-zA-Z0-9]{2,4}$/', $uri) || $this->lang() == $this->default_uri)
		{
